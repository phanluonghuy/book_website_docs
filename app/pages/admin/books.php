	...
	if($action == 'add')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			if(empty($errors))
			{
				...
				$query = "insert into books (title,image,file,user_id,category_id,artist_id,date,views,slug) values (:title,:image,:file,:user_id,:category_id,:artist_id,:date,:views,:slug)";
	...
	if($action == 'edit')
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST" && $row)
		{
			...
			if(empty($errors))
			{
				...
				$query = "update books set title = :title,user_id =:user_id,category_id =:category_id,artist_id =:artist_id";
...
<?php require page('includes/admin-header')?>
	...
	<section class="admin-content" style="min-height: 200px; background-color: #f7f7f7;">
  		...
  		<?php if($action == 'add'):?>
  			...
  			<div style="max-width: 500px;margin: auto;">
	  			<form method="post" enctype="multipart/form-data">
	  				...
	  				<input name="title" class="form-control my-1" value="<?=set_value('title')?>" type="text" title="title" placeholder="Book title">
  			...
  			<div style="max-width: 500px;margin: auto;">
	  			<form method="post" enctype="multipart/form-data">
	  				...
	  				<?php if(!empty($row)):?>
	  					  				...
	  					  				<input name="title" class="form-control my-1" value="<?=set_value('title',$row['title'])?>" type="text" title="title" placeholder="Book title">
	  				<?php if(!empty($errors['title'])):?>
	  					<small class="error"><?=$errors['title']?></small>
	  				<?php endif;?>
  		...
  		<?php else:?>
  			...
  			<?php
				...
				$offset = ($page - 1) * $limit;
  				...
  				$query = "select * from books order by id desc limit $limit offset $offset";
  				$rows = db_query($query);

  			?>
  			<h3>Books
  				<a href="<?=ROOT?>/admin/books/add">
  				...
  				<?php if(!empty($rows)):?>
	  				<?php foreach($rows as $row):?>
		  				<tr>
		  					...
		  					<td>
		  						<a href="<?=ROOT?>/admin/books/edit/<?=$row['id']?>">
		  							<img class="bi" src="<?=ROOT?>/assets/icons/pencil-square.svg">
		  						</a>
		  						<a href="<?=ROOT?>/admin/books/delete/<?=$row['id']?>">
  	...
  	<div class="mx-2">
		<a href="<?=ROOT?>/admin/books?page=<?=$prev_page?>">
			<button class="btn bg-orange">Prev</button>
		</a>
		<a href="<?=ROOT?>/admin/books?page=<?=$next_page?>">
